...
namespace MKRecorder
{
    ...
    {
        private MyHook.MouseEventFlags flag;
        private int x, y;

        public MyHook.MouseEventFlags Flag
        {
            get { return flag; }
            set { flag = value; }
        }
        ...
        private static void MouseEvent(MyHook.MouseEventFlags f, int X, int Y)
        {
            SetCursorPos(X, Y);
            if (f != MyHook.MouseEventFlags.MO)
            {
                mouse_event((uint)f, (uint)X, (uint)Y, 0, 0);
            }
        }

        public MouseAction(MyHook.MouseEventFlags f, int x, int y, int time)
        {
            this.flag = f;
            this.x = x;
            this.y = y;
        ...
        public MouseAction(string str)
        {
            ...
            x = int.Parse(param[2]);
            y = int.Parse(param[3]);
            flag = (MyHook.MouseEventFlags)Enum.Parse(typeof(MyHook.MouseEventFlags), param[4]);
        }
