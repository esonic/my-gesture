...
namespace CSharpInterpreter
{
    ...
    public class CSharpInterpreter
    {
        ...
        /// <summary>
        /// 执行源代码
        /// 类必须命名为 CSharpExecutor.Executor 且具有方法 public Execute(string[] args)
        /// </summary>
        ...
        {
            string src = "using System;using System.Collections.Generic;using System.Text;using System.IO;using System.Drawing;using System.Threading;using System.Linq;using Common;using System.Windows.Forms;" +
        ...
        private static string Run(CompilerResults result, string[] args)
        {
            if (result.Errors.Count == 0)
            {
                ...
                {
                    throw new ExecuteException("Execute failed, see inner exception for details", ex);
                }
            }
            else
            {
        ...
        private static CompilerParameters GetCompileParam(string[] referDlls)
        {
            ...
            string cpath = System.IO.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) +"\\Common.dll";
            compilerParameters.ReferencedAssemblies.Add(cpath);
